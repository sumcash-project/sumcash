...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTimeTx, uint32_t nTimeBlock, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
    ...
    // uint256 gen = genesis.GetHash();
    // uint256 target = bnTarget.getuint256();
    // printf("gen:\n%s\ntarget:\n%s", gen.ToString().c_str(), target.ToString().c_str());
    // while (gen > target)
    // {
...
// -        consensus.bnInitialHashTarget = uint256S("0000000000ffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~arith_uint256(0) >> 40;
        ...
        genesis = CreateGenesisBlock(1554579000, 1554579000, 2088111234u, 0x1e0ffff0, 1, 10000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        printf("genesis:\n%s\npow:\n%s\nhashTarget:\n%s\n",consensus.hashGenesisBlock.ToString().c_str(), consensus.powLimit.ToString().c_str(), consensus.bnInitialHashTarget.ToString().c_str());
        printf("assert:\n%s\n", uint256S("0x8207cc10210f8c1cc2c30d435e637330eb27576bcd126e823e95a81da8769008").ToString().c_str());
        assert(consensus.hashGenesisBlock == uint256S("0x8207cc10210f8c1cc2c30d435e637330eb27576bcd126e823e95a81da8769008"));
        ...
        chainTxData = ChainTxData{
            ...
            1555173583, // * UNIX timestamp of last known number of transactions
            10002,    // * total number of transactions between genesis and that timestamp
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(1554578100, 1554578100, 387568175, 0x1e0ffff0, 1, 10000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0xb588cbcc15cc16749c3799e401d758b28bee816fbdfce137064aeb44e425332f"));
        assert(genesis.hashMerkleRoot == uint256S("0xa14add4166fd2eb3c8f52566d23b68438094f4a72e359ad7f92506f0f813ea2f"));
